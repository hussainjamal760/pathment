...
## Backend
- **Matrix:** `config/notificationMatrix.js` — ~25 events, each with a `preferenceKey` and default channels (in-app / email / chat). It also exports `EMAIL_PREFERENCE_CATEGORIES` (the user-toggleable, non-transactional emailable events, grouped).
